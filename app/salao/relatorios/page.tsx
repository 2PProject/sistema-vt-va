...
export default function RelatoriosPage(){
 ...
 const exibidos=useMemo(()=>profissionais.filter(r=>{if(aba==='pendentes'&&concluido(r))return false;if(aba==='conferidos'&&!concluido(r))return false;if(profissional&&r.nome!==profissional)return false;if(busca&&!String(r.nome||'').toLocaleLowerCase('pt-BR').includes(busca.toLocaleLowerCase('pt-BR')))return false;return true}).sort((a,b)=>String(a.nome||'').localeCompare(String(b.nome||''),'pt-BR')),[profissionais,aba,profissional,busca])
 ...
 return <LayoutAdmin title="Salão — Relatório de conferência"><div className="space-y-3">
  ...
  {!inicio||!fim?<Vazio titulo="Informe o período" texto="Escolha o mês inicial e o mês final para montar o relatório."/>:<div className="grid gap-3 xl:grid-cols-[1.35fr_.65fr]">
   ...
   <section className="overflow-hidden rounded-xl border bg-white shadow-sm"><div className="border-b p-3"><h2 className="font-semibold">Notas sem vínculo</h2><p className="text-xs text-slate-500">{periodoLabel(inicio,fim)} · {notasLivres.length} nota(s)</p></div>{loading&&linhas.length===0?<Carregando/>:notasLivres.length===0?<Vazio titulo="Nenhuma nota sem vínculo" texto="Todas as notas do recorte estão tratadas."/>:<div className="max-h-[620px] overflow-auto divide-y">{notasLivres.map(r=><article key={r.id} className="p-3"><div className="flex justify-between gap-2"><b className="text-sm">{r.nome||'Emitente não identificado'}</b><b className="text-sm">{formatarMoeda(r.nf_valor||0)}</b></div><p className="mt-1 text-xs text-slate-500">{r.documento||'Documento não informado'} · NF {r.nf_numero||'—'}</p><p className="mt-2 text-xs text-amber-800"><b>Motivo:</b> nenhum profissional importado foi vinculado a esta nota.</p><p className="mt-1 text-[11px] text-slate-400">Emissão {fmtData(r.nf_data)} · {r.empresaNome}</p></article>)}</div>}</section>
  </div>}
 </div></LayoutAdmin>
}
