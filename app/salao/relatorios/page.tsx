...
export default function RelatoriosPage(){
 ...
 async function pdfPendencias(){const pro=profissionais.filter(r=>!concluido(r)&&(profissional?r.nome===profissional:true));const rows=[...pro.map(r=>({cnpj:r.documento||'',nome:r.nome||'',competencia:r.mes_ref,valor:r.valor_comissao||0})),...notasExibidas.map(r=>({cnpj:r.documento||'',nome:r.nome||'',competencia:r.mes_ref,valor:r.nf_valor||0}))];if(!rows.length)return;const cols:Coluna[]=[{header:'CNPJ / CPF',get:r=>r.cnpj},{header:'Nome',get:r=>r.nome},{header:'Valor',get:r=>formatarMoeda(r.valor)},{header:'Competência',get:r=>fmtMes(r.competencia)}];await exportarPDF(`Pendências de notas — ${periodoLabel(inicio,fim)}`,cols,rows,`pendencias_notas_${inicio}_${fim}`,{orientation:'portrait'})}
