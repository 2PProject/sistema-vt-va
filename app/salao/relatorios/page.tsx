...
export default function RelatoriosPage(){
 const router=useRouter();const[competencia,setCompetencia]=useState('');const[empresaId,setEmpresaId]=useState('');const[empresas,setEmpresas]=useState<Empresa[]>([]);const[linhas,setLinhas]=useState<LinhaConsulta[]>([]);const[aba,setAba]=useState<Aba>('pendentes');const[busca,setBusca]=useState('');const[loading,setLoading]=useState(false);const[erro,setErro]=useState('');const req=useRef(0)
 ...
 const carregar=useCallback(async()=>{const id=++req.current;if(!competencia){setLinhas([]);setLoading(false);return}setLoading(true);setErro('');try{const dados=await buscarTudo(competencia,empresaId);if(id===req.current)setLinhas(dados)}catch(e){if(id===req.current)setErro(e instanceof Error?e.message:'Erro ao consultar o período.')}finally{if(id===req.current)setLoading(false)}},[competencia,empresaId])
 ...
 const exibidos=useMemo(()=>profissionais.filter(r=>{if(aba==='pendentes'&&concluido(r))return false;if(aba==='conferidos'&&!concluido(r))return false;if(busca&&!String(r.nome||'').toLocaleLowerCase('pt-BR').includes(busca.toLocaleLowerCase('pt-BR')))return false;return true}).sort((a,b)=>String(a.nome||'').localeCompare(String(b.nome||''),'pt-BR')),[profissionais,aba,busca])
 function limpar(){req.current++;setCompetencia('');setEmpresaId('');setBusca('');setAba('pendentes');setLinhas([]);setErro('');setLoading(false)}
 ...
 return <LayoutAdmin title="Salão — Relatório de conferência"><div className="space-y-3">
  ...
  <section className="rounded-xl border bg-white p-4 shadow-sm"><div className="flex flex-wrap items-end gap-3"><div className="mr-auto"><h1 className="text-base font-semibold text-slate-900">Situação das notas por período</h1><p className="text-xs text-slate-500">Quem está pendente, o motivo e quais notas ainda não têm vínculo.</p></div><div><label className="label-field">Competência</label><input type="month" className="input-field" value={competencia} onChange={e=>setCompetencia(e.target.value)}/></div><div className="min-w-56"><label className="label-field">Unidade</label><select className="input-field" value={empresaId} onChange={e=>setEmpresaId(e.target.value)}><option value="">Todas as unidades</option>{empresas.map(e=><option key={e.id} value={e.id}>{e.apelido||e.razao_social}</option>)}</select></div><button className="btn-secondary text-sm" onClick={limpar}>Limpar</button><button className="btn-primary text-sm print:hidden" disabled={!competencia} onClick={()=>window.print()}>Imprimir</button></div>{loading&&linhas.length>0&&<p className="mt-2 text-xs text-blue-600">Atualizando em segundo plano…</p>}</section>
