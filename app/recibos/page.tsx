...
import { supabase, Competencia, CompetenciaFuncionario, Funcionario, Unidade, Empresa } from '../../lib/supabase'
import { calcularVTVA, formatarMoeda, MESES } from '../../utils/calculoVT'

type CFComFuncionario = CompetenciaFuncionario & {
  funcionarios: Funcionario & { unidades: Unidade & { empresas: Empresa } }
}

export default function RecibosPage() {
  const [unidades, setUnidades] = useState<Unidade[]>([])
  const [competencia, setCompetencia] = useState<Competencia | null>(null)
  const [registros, setRegistros] = useState<CFComFuncionario[]>([])
  ...
  async function carregarUnidades() {
    const { data } = await supabase.from('unidades').select('*, empresas(*)').order('nome')
  ...
  const buscarDados = useCallback(async () => {
    ...
    const { data: cfs } = await supabase
      ...
      .select('*, funcionarios(*, unidades(*, empresas(*)))')
      .eq('competencia_id', comp.id)

    setRegistros((cfs as CFComFuncionario[]) ?? [])
    setLoading(false)
  }, [unidadeId, mes, ano])

  useEffect(() => {
    if (unidadeId) buscarDados()
  }, [unidadeId, mes, ano, buscarDados])

  async function gerarPDF(cf: CFComFuncionario) {
    setGerando(cf.funcionario_id)
    try {
      const { gerarReciboPDF } = await import('../../services/gerarReciboPDF')
      const func = cf.funcionarios
      const unidade = func.unidades
      ...
      <div className="space-y-6">
                        ...
                        {registros.map((cf) => {
                          ...
                          return (
                            <tr key={cf.funcionario_id} className="hover:bg-gray-50 transition-colors">
                              ...
                              <td className="table-cell text-right text-sm">{formatarMoeda(resultado.totalVA)}</td>
                              <td className="table-cell text-right text-sm">{formatarMoeda(resultado.totalVT)}</td>
                              <td className="table-cell text-right text-sm">{formatarMoeda(resultado.totalVTSabado)}</td>
        ...
        {!unidadeId && (
          <div className="card text-center py-16 text-gray-400">
            ...
            <p className="text-sm">Selecione uma unidade e período para visualizar e gerar recibos.</p>
          </div>
        )}
      </div>
    </LayoutAdmin>
